...
import { customerAgentLinksRepository } from "~/lib/customer-agent-links/customer-agent-links.repo";
import { creditAllocationsRepository } from "~/lib/credit-allocations/credit-allocations.repo";
import { agentFeeTransactionsRepository } from "~/lib/agent-fee-transactions/agent-fee-transactions.repo";
import { SIGNAL_TYPES, FEE_TYPES, COST_TYPES, type CostType, type AgentFeeTransaction } from "~/lib/database/server";
  ...
  POST: async ({ request }) => {
    try {
      ...
      if (isRestricted) {
        ...
        if (agent.setupFeeEnabled && !feeTransactions.some((t: AgentFeeTransaction) => t.feeType === FEE_TYPES.SETUP)) {
